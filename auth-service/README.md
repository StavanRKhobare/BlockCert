...
- [B5] Public entrypoint score_client (exact integration signature; B1 amended to also return raw "embeddings" for the micro-cluster signal; test uses the real calibrated threshold, no monkeypatch) — files: auth-service/entrypoint.py, auth-service/tests/test_entrypoint.py, auth-service/golden_reference/reference_stats.py, auth-service/tests/test_reference_stats.py.
